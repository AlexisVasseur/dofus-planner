...
<template>
  <section class="purchase-planner flex-1 overflow-y-auto relative z-[1] no-scrollbar">
    ...
    <template v-else>
      ...
      <!-- One NPC per row. NPC label on the left, items wrap horizontally on the right.
      ...
      >
        <div
          ...
          :key="npc"
          class="grid items-center gap-x-6 py-2.5"
          :class="idx > 0 && 'border-t border-border-subtle'"
          style="grid-template-columns: 120px 1fr;"
        >
          <div class="font-sans font-bold text-[11px] text-text-muted tracking-[0.08em] uppercase">
            {{ NPC_LABEL[npc] }}
          </div>
          <div class="flex flex-wrap gap-x-5 gap-y-1.5">
            <button
              ...
              class="inline-flex items-center gap-1.5 text-left text-[13px] font-sans whitespace-nowrap text-text-default hover:text-[#8AE0EE] transition-colors cursor-pointer"
            ...
            >
              <img
                ...
                loading="lazy"
              />
              <span>{{ copiedKey === `${npc}-${item.id}` ? '✓ ' : '' }}{{ item.name }}</span>
            </button>
          </div>
        </div>
      </div>
